...
const Projects = ({ slides }) => {
    ...
    return (
        <div role="region" className="w-full h-max mt-20 py-2 bg-white md:mt-28">
            ...
            <div className="w-full h-max relative px-2 mt-7 md:mt-10">
                {sliderData.map((slide, index) => {
                    return (
                        <div className={index === current ? 'opacity-100 transition duration-700 ease-in' : 'opacity-0'} key={index}>
                            {index === current && (
                            <div className="flex flex-col justify-center items-center">
                                <img src={slide.image} className="h-44 object-cover rounded-md xs:h-48 md:w-full md:h-96" alt="Project images"/>
